...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await db.item.findUnique({ where: { id: params.id }, select: { imageStoragePath: true } });
  ...
  const item = await db.item.update({ where: { id: params.id }, data: { imageStoragePath: path } });
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await db.item.update({ where: { id: params.id }, data: { imageStoragePath: null } });
  return ok();
}
