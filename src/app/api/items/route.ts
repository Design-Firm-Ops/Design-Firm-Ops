...
export async function POST(req: NextRequest) {
  ...
  const itemTypeId = await findOrCreateItemType(data.category, itemType, firmId);
  if (rest.room) await findOrCreateRoom(data.projectId, rest.room, firmId);
  ...
  const maxSort = await db.item.aggregate({
